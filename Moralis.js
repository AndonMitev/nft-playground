...
const { createTransferInstruction, createAccount, getAssociatedTokenAddress, getAccount, TokenAccountNotFoundError } = require('@solana/spl-token')
...
const mainnet_rpc = 'https://solana--mainnet.datahub.figment.io/apikey/d7d9844ccf72ad4fef9bc5caaa957a50'
...
(async () => {
  const nfts = await Moralis.Web3API.account.getNFTs({
    address: '0x7fB3d24Db64c9396ac132d10397a95543950e320',
    chain: 'polygon',
  });

  // 

  const asd = nfts.result.filter(nft => nft.token_address === '0x2953399124f0cbb46d2cbacd8a89cf0599974963' && nft.token_id === '79878392158992066930965209155561399463522637681325877961483264265116352349855')
